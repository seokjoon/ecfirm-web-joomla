...
class ModEcgithubHelper
{
	...
	private static function _getActivityEvent($params, $type)
	{
		...
		switch ($type) {
			case 'commits':
			case 'default':
				$src = $git->activity->events->getRepository($params->get('username'), $params->get('repository'));
				foreach ($src as $s)
					if ($s->type == 'PushEvent')
						array_push($out, $s);
				break;
			case 'issues':
				$src = $git->activity->events->getIssue($params->get('username'), $params->get('repository'));
				$mapNumber = array();
				foreach ($src as $s) {
					if (in_array($s->issue->number, $mapNumber))
						continue;
