...
class EcuserControllerLogin extends EcControllerForm
{
	...
	public function login()
	{
		...
		if (true !== $app->login($data, array()))
			$this->setRedirectParams(array(
				'task' => 'login.useForm'
			));
		else 
			if ((empty($data['return'])) || (! (JUri::isInternal($data['return']))))
				$this->setRedirectParams(array(
					'view' => 'user',
					'layout' => 'default',
				...
				$app->redirect(JRoute::_($data['return']));
	}
	...
	public function logout()
	{
		...
		if ($error instanceof Exception)
			$this->setRedirect($this->getRedirectRequest()); //@attention
		else 
			if ((empty($data['return'])) || (! (JUri::isInternal($data['return']))))
				$this->setRedirectParams(array(
					'task' => 'login.useForm'
