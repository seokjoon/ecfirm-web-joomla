		...
		echo '<table class="table table-striped" id="articleList">';
			echo '<thead><tr>';
				...
				echo '<th class="center">';
					echo JText::_('COM_ECTOPIC_TOPICCAT_TITLE_HEADER');
				echo '</th>';
				echo '<th class="center">';
					echo JText::_('COM_ECTOPIC_TOPICCAT_STATE_HEADER');
				echo '</th>';
			echo '</tr></thead>';
			echo '<tbody>';
			foreach($this->items as $i => $item)	{
				echo '<tr class="row'.($i % 2).'" sortable-group-id="'.$item->topiccat.'">';
					echo '<td class="center hidden-phone">';
						echo JHtml::_('grid.id', $i, $item->topiccat);
					echo '</td>';
					echo '<td class="has-context">';
						echo '<a href="'.JRoute::_
							('index.php?option=com_ectopic&task=topiccat.edit&topiccat='
							.$item->topiccat).'" title="'.JText::_('JACTION_EDIT').'">';
							echo $item->title.'</a>';
					echo '</td>';
					echo '<td class="center hidden-phone">';
