...
class EctopicTableTopicObserver extends JTableObserver
{
	...
	public function onBeforeStore($updateNulls, $tableKey)
	{
		...
		if (empty($this->table->topic))
			$this->table->created = $this->table->modified;
	...
	private function updateFile($name = 'files')
	{
		...
		if ((! empty($this->table->$name)) && ($this->$nameLoad != $this->table->$name)) { //EcDebug::lp($this->$nameLoad);; EcDebug::lp($this->table->$name, true);
			$reg = new JRegistry();
			if (! empty($this->$nameLoad))
				$reg->loadString($this->$nameLoad);
			$this->table->$name = (empty($this->table->$name)) ? array() : json_decode($this->table->$name, true);
			foreach ($this->table->$name as $fileName => $fileValue)
				if (! empty($fileValue)) {
					EcFile::delete(array(
						$reg->get($fileName)
					)); //EcDebug::lp(array($reg->get($fileName)), true);
