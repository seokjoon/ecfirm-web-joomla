...
class EcModelItem extends ItemModel //JModelItem
{
	...
	public function delete(&$valueKeys)
	{
		...
		foreach ($valueKeys as $i => $valueKey) {
			...
			if ($this->getState('enabledPlugin', false)) {
				$result = $dispatcher->trigger('on' . ucfirst($this->name) . 'BeforeDelete', array(
					$this->context,
					$table
				));
			...
			if ($this->getState('enabledPlugin', false))
				$dispatcher->trigger('on' . ucfirst($this->name) . 'AfterDelete', array(
					$this->context,
					$table
				));
		}
	...
	{ //EcDebug::log($data, __method__);
		...
		try {
			...
			if ($this->getState('enabledPlugin', false)) {
				$resule = $dispatcher->trigger('on' . $this->name . 'BeforeSave', array(
					$this->context,
					$table,
					$isNew
				));
