...
class EcModelForm extends JModelForm
{
	...
	{
		if ($valueKey == 0)
			$valueKey = JFactory::getApplication()->input->get($this->nameKey, 0, 'uint');
		if ($valueKey == 0)
			return false;
	...
	protected function preprocessData($context, &$data, $group = 'ec')
	{
		...
		$results = $dispatcher->trigger('on' . $this->nameKey . 'PrepareData', array(
			$context,
			$data
		));
	...
	protected function preprocessForm(JForm $form, $data, $group = '')
	{
		...
		if (empty($group))
			$group = EcConst::getPrefix(); //'ec';
		
		JPluginHelper::importPlugin($group);
		$dispatcher = JEventDispatcher::getInstance();
		
		$results = $dispatcher->trigger('on' . $this->nameKey . 'PrepareForm', array(
			$form,
			$data
		));
