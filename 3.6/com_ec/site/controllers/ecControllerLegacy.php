...
class EcControllerLegacy extends JControllerLegacy {
	 ...
	 * @return  boolean
	 * @since   12.2 JControllerForm */
	protected function allowAdd($data = array()) { //XXX
	...
	protected function getRedirectReturn() {
		...
		if(empty($return)) return JUri::base();
		else return base64_decode($return);
	}
	
	protected function getUser() { return JFactory::getUser()->id; }
	...
	protected function save($nameKey = null, $urlVar = null) { 
		...
		////////
		if(($nameKey != 'user') && (isset($data['user'])) && ($data['user'] == 0)) 
			$data['user'] = JFactory::getUser()->id; //ATTENTION
		////////
		////////
	...
	protected function setRedirectParams($params = array()) {
		...
		$option = (isset($params['option'])) ? '?option=com_'.$params['option'] : 
			$option = '?option=com_'.$this->name;
		$view = (isset($params['view'])) ? '&view='.$params['view'] : 
